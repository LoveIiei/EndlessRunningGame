...
export class Rolling extends State {
    ...
    handleInput(input) {
        if (this.game.energy > 0) this.game.energy = this.game.energy - 0.3;
        if (this.game.energy <= 1) this.game.player.setState(states.RUNNING, 1);
        this.game.particles.unshift(new Fire(this.game, this.game.player.x +
            this.game.player.width * 0.5,
            this.game.player.y + this.game.player.height * 0.5));
        if (!input.includes('Enter') && this.game.player.onGround()) {
            this.game.player.setState(states.RUNNING, 1);
        } else if (!input.includes('Enter') && !this.game.player.onGround() && this.game.energy > 0) {
            this.game.player.setState(states.FALLING, 1);
        } else if (input.includes('w') && input.includes('Enter') &&
